...
type Context = { params: Promise<{ id: string; valueId: string }> };
async function proxy(request: NextRequest, context: Context, method: "PATCH" | "DELETE") {
  ...
  if (!/^\d+$/.test(id) || !/^\d+$/.test(valueId)) return NextResponse.json({ message: "Invalid attribute value ID." }, { status: 400 });
  try {
    const response = await fetch(getAdminApiUrl(`admin/product-attributes/${id}/values/${valueId}`), { method, headers: { Authorization: `Bearer ${token}`, ...(method === "PATCH" ? { "Content-Type": "application/json" } : {}) }, ...(method === "PATCH" ? { body: await request.text() } : {}), cache: "no-store" });
