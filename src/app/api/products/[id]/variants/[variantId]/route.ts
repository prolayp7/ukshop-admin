...
async function proxy(request: NextRequest, context: Context, method: "PATCH" | "DELETE") {
  ...
  try {
    const response = await fetch(getAdminApiUrl(`admin/products/${id}/variants/${variantId}`), { method, headers: { Authorization: `Bearer ${token}`, ...(method === "PATCH" ? { "Content-Type": "application/json" } : {}) }, ...(method === "PATCH" ? { body: await request.text() } : {}), cache: "no-store" });
