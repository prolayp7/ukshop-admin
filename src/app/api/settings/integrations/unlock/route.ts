...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await fetch(getAdminApiUrl("admin/settings/integrations/unlock"), { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: await request.text(), cache: "no-store" });
