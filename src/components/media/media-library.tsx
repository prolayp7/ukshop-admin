...
function UploadDialog({ collections, onClose, onComplete }: { collections: string[]; onClose: () => void; onComplete: () => void }) {
  ...
  function acceptFiles(nextFiles: File[]) {
    setError("");
    const invalid = nextFiles.find((file) => !acceptedTypes.includes(file.type) || file.size > 1024 * 1024);
    if (invalid) { setError(`${invalid.name} is unsupported or larger than 1 MB.`); return; }
    setFiles(nextFiles);
  }
