...
function money(value: string | number) { return new Intl.NumberFormat("en-GB", { style: "currency", currency: "GBP" }).format(Number(value)); }
...
export function CustomersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = unwrap<{ items: Customer[]; meta: Meta }>(customersPayload);
      setItems(data.items ?? []); setMeta(data.meta ?? emptyMeta); setSelected(new Set());
  ...
  async function exportCustomers() {
    ...
    try {
      ...
      const exported = unwrap<{ items: Customer[] }>(payload).items ?? [];
  ...
  return <div className="pb-6">
    ...
    {detailId ? <CustomerDrawer id={detailId} onClose={() => setDetailId(null)} /> : null}
  </div>;
}
