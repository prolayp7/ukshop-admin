...
type Tab = "description" | "details" | "stock" | "shipping" | "seo";
...
type ProductDetail = {
  ...
  variants: { price: string; salePrice: string | null; stockQty: number; lowStockThreshold: number; weightKg: string | null; widthCm: string | null; heightCm: string | null; lengthCm: string | null }[];
};
...
export function NewProductPage({ productType, productId }: { productType: ProductType; productId?: number }) {
  ...
  async function submit(event: FormEvent, publish: boolean) {
    ...
    if (productType === "standard" && salePrice && Number(salePrice) >= Number(retailPrice || 0)) {
  ...
  {activeTab === "stock" ? <div className="space-y-7">
    {productType === "variants" ? <div className="rounded-lg bg-accent-tint p-4 text-[13px] leading-5 text-accent-tint-ink ring-1 ring-inset ring-accent-tint-border">This product uses combinations. Save the catalogue record first, then set price and stock for each variant.</div> : <>
      <section><div className="mb-3"><h2 className="text-sm font-semibold text-ink">Retail price</h2><p className="mt-1 text-xs text-ink-muted">Enter the price excluding VAT. The customer-facing GBP price is calculated automatically.</p></div><div className="grid gap-4 md:grid-cols-3">
        <label className={labelClass}><span className="inline-flex items-center gap-1">Retail price (excl. VAT)<HelpTooltip label="Retail price excluding VAT">The net selling price before UK VAT. Prices are stored in GBP.</HelpTooltip></span><div className="relative mt-2"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-ink-muted">£</span><input type="number" min="0" step="0.01" value={retailPrice} onChange={(event) => setRetailPrice(event.target.value)} placeholder="0.00" className={`${inputClass} mt-0 pl-7`} /></div></label>
        ...
        <label className={labelClass}>Retail price (incl. VAT)<div className="mt-2 flex h-10 items-center rounded-md border border-border bg-neutral-tint px-3 text-[13px] font-semibold text-ink">£{priceIncVat.toFixed(2)}</div></label>
        <label className={labelClass}><span className="inline-flex items-center gap-1">Cost price (excl. VAT)<HelpTooltip label="Cost price">Your supplier or acquisition cost before VAT. This is used only for margin reporting and is never shown to customers.</HelpTooltip></span><div className="relative mt-2"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-ink-muted">£</span><input type="number" min="0" step="0.01" value={costPrice} onChange={(event) => setCostPrice(event.target.value)} placeholder="0.00" className={`${inputClass} mt-0 pl-7`} /></div></label>
        <label className={labelClass}><span className="inline-flex items-center gap-1">Sale price (optional)<HelpTooltip label="Sale price">A temporary customer price including any discount. It must be lower than the normal price.</HelpTooltip></span><div className="relative mt-2"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-ink-muted">£</span><input type="number" min="0" step="0.01" value={salePrice} onChange={(event) => setSalePrice(event.target.value)} placeholder="No sale price" className={`${inputClass} mt-0 pl-7`} /></div></label>
      </div><div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-md bg-neutral-tint p-3"><p className="text-[10.5px] font-semibold uppercase tracking-wide text-ink-muted">Price incl. VAT</p><p className="mt-1 text-lg font-semibold text-ink">£{priceIncVat.toFixed(2)}</p></div><div className="rounded-md bg-neutral-tint p-3"><p className="text-[10.5px] font-semibold uppercase tracking-wide text-ink-muted">Gross margin</p><p className="mt-1 text-lg font-semibold text-ink">£{margin.toFixed(2)}</p></div><div className="rounded-md bg-neutral-tint p-3"><p className="text-[10.5px] font-semibold uppercase tracking-wide text-ink-muted">Margin rate</p><p className="mt-1 text-lg font-semibold text-ink">{marginRate.toFixed(1)}%</p></div></div></section>
  ...
  {activeTab === "shipping" ? <div className="space-y-7">
    <section><div className="mb-3"><h2 className="inline-flex items-center gap-1 text-sm font-semibold text-ink">Package dimensions<HelpTooltip label="Package dimensions">Enter the packed dimensions and weight used by carrier rate and size restrictions, not the unpacked product measurements.</HelpTooltip></h2><p className="mt-1 text-xs text-ink-muted">Measure the parcel including retail packaging and protective materials.</p></div>{productType === "variants" ? <div className="rounded-md bg-accent-tint p-4 text-xs leading-5 text-accent-tint-ink ring-1 ring-inset ring-accent-tint-border">Dimensions and weight are assigned to each combination after this catalogue record is saved.</div> : <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[["Width", packageWidth, setPackageWidth, "cm"], ["Height", packageHeight, setPackageHeight, "cm"], ["Depth", packageDepth, setPackageDepth, "cm"], ["Weight", packageWeight, setPackageWeight, "kg"]].map(([label, value, setter, unit]) => <label key={label as string} className={labelClass}>{label as string}<div className="relative mt-2"><input type="number" min="0" step={unit === "kg" ? "0.001" : "0.01"} value={value as string} onChange={(event) => (setter as (value: string) => void)(event.target.value)} placeholder="0" className={`${inputClass} mt-0 pr-10`} /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-ink-muted">{unit as string}</span></div></label>)}</div>}</section>
